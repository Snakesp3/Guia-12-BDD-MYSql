...
o Utilizando la función YEAR de MySQL. o Utilizando la función DATE_FORMAT de MySQL. o Sin utilizar ninguna de las funciones anteriores.
*/
 SELECT DISTINCT
    c.codigo_cliente, c.nombre_cliente, fecha_pago
FROM
    cliente c
        INNER JOIN
...
*/
SELECT 
    codigo_empleado_rep_ventas, nombre_cliente, ciudad
FROM
    cliente
WHERE
    ciudad = 'Madrid'
...
*/
SELECT 
    nombre_cliente AS NombreCLientes,
    nombre AS Nombre,
...
FROM
    cliente
        INNER JOIN
    empleado ON codigo_cliente = codigo_empleado
WHERE
    puesto = 'Representante ventas';
/*
...
representantes de ventas.
*/
select codigo_cliente,nombre_cliente as NombreCLiente
/*
